# extract_invariants: moving fields that are invariant per key into a lookup table
...
- On `chunk_2-00114`, every `labels`/`property-labels`/`unit-labels` object has its sibling key (`id`/`property`/`unit`), and no key maps to two different subtrees — each label-map field is invariant per its sibling key, which determines it (in database terms, a functional dependency `id → labels` with `id` as the determinant).
...
- An `extract_invariants: dict[str, str]` option on `normalise_from_parquet`, mapping each field that is invariant per key to its determinant (e.g. `{"labels": "id", "property-labels": "property", "unit-labels": "unit"}`), under which each object holding both fields has the field removed before inference and normalisation.
...
- A policy for a field that is not invariant (one determinant value with two different values): an error, or the first value kept with a count of conflicts.
...
- Timing and peak-RSS measurements of `normalise_from_parquet` with and without `extract_invariants` on a Wikidata chunk.
- `extract_invariants` on `normalise_json`, `infer_json_schema` and `infer_from_parquet`.
