...
- Four further changes after a1bca40 — a `SchemaNode::add_schema` path without `get_subschemas`' Vec, `SchemaNode::to_schema` collecting scalar leaves in a Vec instead of a `HashSet<String>` plus a fresh `json!` value (1.78s to 1.53s), streamed xxh64 of the row schema, and `ObjectStrategy::add_object` without a per-object key set plus skipping `apply_force_field_types` when no field is forced — take the claims slice from 1.90s to 1.48–1.50s (genson-core/src/genson_rs/node.rs, strategy/object.rs, genson-core/src/schema.rs)
- `bench_claims` against a build of 4cc6916 (0.7.6) on four chunks, two runs each: `chunk_3-00031` 3.12s to 1.49s, `chunk_2-00114` 1.70s to 0.96s, `chunk_5-00037` 4.15s to 2.11s, `chunk_0-00283` 4.12s to 2.15s
- The 0.7.6 build prints different schema hashes on repeated runs of `chunk_2-00114`, `chunk_5-00037` and `chunk_0-00283` — the schemas parse to equal JSON (object key order ignored, array order kept) between two 0.7.6 runs and between 0.7.6 and the current branch on all four chunks, so `DUMP=path` in `bench_claims` writes the schema for that comparison
- On the 0.7.7 code (which contains the changes above) the claims slice takes 1.50s wall, split into 620 ms summed parallel phase, 710 ms summed serial merge and a 150 ms tail — the tail is 50 ms for the final `SchemaBuilder::to_schema` and 96 ms for `rewrite_objects` (genson-core/src/schema.rs)
- `ObjectStrategy::add_schemas_par` merges a chunk's row schemas with one rayon task per root property, feeding each property node its sub-schemas in row order, and `SchemaNode::add_schemas_par` recurses through plain object schemas and plain array schemas (`items` an object) for nodes receiving at least 8 schemas (genson-core/src/genson_rs/strategy/object.rs, node.rs, strategy/array.rs, builder.rs) — the claims slice takes 1.20s with the root-level split and 1.12s with the recursion, and the summed merge falls from 706 ms to 285 ms
- With the root-level split alone the per-chunk parallel merge wall time (250 ms summed) equals the sum of each chunk's slowest property (246 ms), against 1073 ms of summed per-property time
- `bench_claims` on four chunks with the recursion, five runs each, against a 0.7.6 build: `chunk_3-00031` 3.12s to 1.12s, `chunk_2-00114` 1.70s to 0.77s, `chunk_5-00037` 4.15s to 1.80s, `chunk_0-00283` 4.12s to 1.74s, and the schemas parse to equal JSON (object key order ignored) to the 0.7.6 output on all four
- Summed across worker threads the per-row stages on the claims slice cost 0.50 CPU-s validation, 3.75 CPU-s build, 4.1 CPU-s `to_schema`, 0.53 CPU-s dropping the builder and 1.16 CPU-s hashing the row schema
- `max_builders` 400 and 1000 give 1.12s and 1.06s on the claims slice against 1.20s at 100 (root-level split only), and 50 gives 1.57s against 1.49s at 100 on the earlier code
...
- Hashing each row schema by walking the `Value` instead of streaming its serialisation cost 1.0 CPU-s against 1.16 CPU-s and left wall time at 1.18s against 1.19s
- Skipping `par_split_mut` and the fold/reduce for a row without a delimiter inside left the claims slice at 1.19–1.20s
- Building `properties` in `ObjectStrategy::properties_to_schema` in parallel for 64 or more properties gave 1.15s against 1.12s, and for 16 or more gave 1.25s

- Removing `add_extra_keywords` from `ObjectStrategy::add_schema` cut the summed serial merge from about 1280 ms to about 1215 ms, and removing the `required` intersection cut it to about 1115 ms — skipping the `required` intersection once the set is empty (`!req.is_empty()`) left the merge at about 960 ms, unchanged, and a linear scan instead of a set for `required` arrays of 8 or fewer keys left the wall time at 1.82s

- Merging chunk N on a separate thread after a1bca40 gave 1.82s against 1.82s, with identical output
