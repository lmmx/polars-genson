...
fn get_delimiter(cli: &Cli) -> Option<u8> {
    ...
    if let Some(delimiter) = d {
        let delimiter = match delimiter {
            ...
            _ => {
                panic!(
                    "Invalid delimiter: {}, must be one of \"newline\", \"tab\", \"space\"",
                    delimiter
                );
            }
        };
...
fn build_schema(builder: &mut SchemaBuilder, file_path: &str, config: BuildConfig) {
...
fn main() {
    ...
    if let Some(json_files) = cli.json_files.as_deref() {
        ...
        let aggregated_builder = json_files
            .par_iter()
            .fold(
