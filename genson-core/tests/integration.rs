...
fn test_invalid_json_integration() {
    ...
    for (invalid_json, description) in test_cases {
        ...
        match result {
            Ok(schema_result) => {
                panic!("Expected error for invalid JSON '{}' but got success: {:?}", invalid_json, schema_result);
            }
            Err(error_msg) => {
                println!("✅ Got expected error: {}", error_msg);
                // Verify it's a proper error message, not a panic message
                assert!(!error_msg.contains("panicked"), "Error message should not contain 'panicked': {}", error_msg);
...
fn test_mixed_valid_and_invalid_json() {
    ...
    match result {
        Ok(schema_result) => {
            ...
            assert!(schema_result.processed_count > 0, "Should have processed at least some JSON");
        }
        Err(error_msg) => {
            println!("✅ Got expected error for mixed input: {}", error_msg);
            // Should be a clean error, not a panic
            assert!(!error_msg.contains("panicked"), "Error should not contain 'panicked': {}", error_msg);
        }
    }
}
