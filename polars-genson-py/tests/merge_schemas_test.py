...
def test_merge_schemas_false():
    ...
    schema_strs = [json.dumps(s, sort_keys=True) for s in schemas]
...
def test_concordant_vs_discordant():
    ...
    concordant_merged = concordant_df.genson.infer_schema("json_data", merge_schemas=True)
    concordant_individual = concordant_df.genson.infer_schema("json_data", merge_schemas=False)
    ...
    discordant_merged = discordant_df.genson.infer_schema("json_data", merge_schemas=True)
    discordant_individual = discordant_df.genson.infer_schema("json_data", merge_schemas=False)
...
def test_expression_with_merge_schemas():
    ...
    merged_schema = json.loads(merged_result.get_column("schema").first())
    ...
    individual_schemas = json.loads(individual_result.get_column("schemas").first())
