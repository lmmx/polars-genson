...
pub fn polars_dtype_to_json_schema(dtype: &DataType) -> Result<Value> {
    match dtype {
        ...
        DataType::Object(_) | DataType::Unknown(_) => {
            Err(ConversionError::UnsupportedPolarsType(format!("{:?}", dtype)))
        }
    }
}
...
pub fn json_schema_to_polars_dtype(json_schema: &Value) -> Result<DataType> {
    ...
    match schema_type {
        ...
        "array" => {
            let items_schema = json_schema
                .get("items")
                .ok_or_else(|| ConversionError::InvalidJsonSchema("Array missing 'items' field".to_string()))?;
        ...
        "object" => {
            let properties = json_schema
                .get("properties")
                .and_then(|p| p.as_object())
                .ok_or_else(|| ConversionError::InvalidJsonSchema("Object missing 'properties' field".to_string()))?;
        ...
        _ => Err(ConversionError::UnsupportedJsonSchemaType(schema_type.to_string())),
    }
}
